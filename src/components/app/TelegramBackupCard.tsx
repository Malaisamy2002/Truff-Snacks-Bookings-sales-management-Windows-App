...
export function TelegramBackupCard() {
  ...
  const [pending, setPending] = useState<{ bytes: Uint8Array; from: string } | null>(null);
  ...
  useEffect(() => {
    void (async () => {
      ...
      setCfg({ ...saved, deviceLabel: saved.deviceLabel || defaultDeviceLabel() });
  ...
  const applyRestore = async (bytes: Uint8Array, from: string) => {
    if (!merge) {
      setPending({ bytes, from });
      return;
    }
    const result = await restoreFullBackup(bytes, "merge");
    await qc.invalidateQueries();
    toast.success(`Restored from ${from}`, { description: restoreSummary(result) });
  };
  ...
  const confirmReplace = async () => {
    ...
    await run("restore", async () => {
      const result = await restoreFullBackup(job.bytes, "replace");
      await qc.invalidateQueries();
      toast.success(`Restored from ${job.from}`, { description: restoreSummary(result) });
    });
  };
  ...
  const backupNow = () =>
    run("backup", async () => {
      ...
      const result = await uploadFullBackup(cfg, bytes, {
        ...
        onProgress: (p) =>
          setProgress(
            p.total === 1 ? "Sending to Telegram…" : `Sending part ${p.part} of ${p.total}…`,
          ),
      });
      const notes = [`${result.parts} file${result.parts === 1 ? "" : "s"} sent`];
      if (missingFiles.length > 0)
        notes.push(`${missingFiles.length} receipt photo(s) weren't on this device`);
      toast.success("Backup sent to your Telegram chat", { description: notes.join(" · ") });
    });
  ...
  const restoreLatest = () =>
    run("fetch", async () => {
      ...
      const { bytes } = await fetchLatestFullBackupArchive(cfg, (p) =>
        setProgress(
          p.total === 1 ? "Downloading the backup…" : `Downloading part ${p.part} of ${p.total}…`,
        ),
      );
  ...
  const showQr = () =>
    run("qr", async () => {
      ...
      setQrDataUrl(await QRCode.toDataURL(encodePairingPayload(cfg), { width: 320, margin: 1 }));
      setQrOpen(true);
    });
  ...
  return (
    <section className="space-y-3">
      ...
      <Card className="frost">
        ...
        <CardContent className="space-y-6">
          <p className="text-sm text-muted-foreground">
            One backup with everything in it — every customer, bill, expense, booking, snack sale{" "}
            <span className="font-medium text-foreground">and</span> every receipt photo — sent to a
            private Telegram chat only you can see. Restoring brings all of it back in one go.
          </p>
          ...
          {!loaded ? (
            ...
            <>
                ...
                <SettingsGrid>
                  <SettingsField label="Bot token" full>
                    <Input
                      type="password"
                      value={cfg.botToken}
                      onChange={(e) => saveCfg({ ...cfg, botToken: e.target.value.trim() })}
                      placeholder="123456:ABC-DEF..."
                    />
                  </SettingsField>
                  <SettingsField label="Chat ID" hint="Usually starts with -100.">
                    <Input
                      value={cfg.chatId}
                      onChange={(e) => saveCfg({ ...cfg, chatId: e.target.value.trim() })}
                  ...
                  <SettingsField label="Name for this device">
                    <Input
                      ...
                      onChange={(e) => saveCfg({ ...cfg, deviceLabel: e.target.value })}
              ...
              >
                <div className="space-y-3">
                  {cfg.extraBotTokens.map((token, i) => (
                    <SettingsField key={i} label={`Extra bot ${i + 2}`} reserveHint={false}>
                      <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            saveCfg({
                              ...cfg,
                              extraBotTokens: cfg.extraBotTokens.filter((_, j) => j !== i),
                            })
                          }
                        >
                  ...
                  <SettingsActions>
                    <Button
                      ...
                      onClick={() =>
                        saveCfg({ ...cfg, extraBotTokens: [...cfg.extraBotTokens, ""] })
                      }
                    >
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {cfg.extraBotTokens.length >= MAX_BOTS - 1 ? " — that's the maximum." : ""}
                  </p>
                </div>
              </SettingsGroup>
              ...
              <SettingsGroup title="Save and pair">
                ...
                <p className="text-xs text-muted-foreground">
                  The bot token never leaves this device. Keep the chat private — anyone in it can
                  read your backups.
                </p>
              </SettingsGroup>
            </>
            ...
            <>
              ...
              <SettingsActions>
                ...
                <Button variant="outline" disabled={busy !== null} onClick={restoreLatest}>
                  <CloudDownload className="mr-1 h-4 w-4" /> Restore latest
                </Button>
                <Button variant="outline" disabled={busy !== null} onClick={showQr}>
          ...
          <div className="frost-well space-y-2 rounded-xl p-3">
            ...
            >
              <HardDriveDownload className="mr-1 h-3.5 w-3.5" /> Restore from a saved file
            </Button>
            <input
              ref={fileRef}
              type="file"
              ...
              onChange={(e) => {
                ...
                if (!file) return;
                void run("local-open", async () =>
                  applyRestore(new Uint8Array(await file.arrayBuffer()), "this device"),
                );
              }}
            />
          </div>
          ...
          <Collapsible open={helpOpen} onOpenChange={setHelpOpen}>
            ...
            <CollapsibleContent className="pt-2">
              <ol className="space-y-1 text-xs text-muted-foreground">
                <li>1. In Telegram, message @BotFather and send /newbot to get a bot token.</li>
                <li>
                  2. Make a private channel or group and add your bot to it as an admin that can
                  post.
                </li>
                <li>
                  3. Send any message there, then paste that chat ID here (it usually starts with
                  -100).
                </li>
                <li>4. Tap "Backup now" — records and receipt photos go over as one file.</li>
                <li>
                  5. On a second device, tap "Scan setup QR" and point it at the first device's
                  "Show setup QR", then tap "Restore latest".
                </li>
              </ol>
              <p className="pt-1 text-xs text-muted-foreground">
                Keep the chat private — anyone in it can read your backups. The bot token stays on
                this device.
              </p>
            </CollapsibleContent>
          </Collapsible>
      ...
      <AlertDialog open={pending != null} onOpenChange={(o) => (o ? undefined : setPending(null))}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Replace everything with this backup?</AlertDialogTitle>
            <AlertDialogDescription>
              This deletes what's on this device now and puts the backup in its place, records and
              receipt photos together. It can't be undone — turn on "Merge with what's already here"
              first if you'd rather add to it.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={qrOpen} onOpenChange={setQrOpen}>
        <DialogContent>
          ...
          {qrDataUrl && (
            <img src={qrDataUrl} alt="Telegram backup setup code" className="mx-auto rounded-lg" />
          )}
          <p className="text-xs text-muted-foreground">
            Open Settings → Telegram backup there and tap "Scan setup QR". Only show this to devices
            you own — it carries the bot token.
          </p>
        </DialogContent>
      </Dialog>

      <QrScannerDialog open={scanOpen} onOpenChange={setScanOpen} onResult={onScanned} />
    </section>
  );
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setError("This device wouldn't share its camera. Type the details in instead.");
      }
    })();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        {error ? (
          ...
          <video ref={videoRef} playsInline muted className="w-full rounded-lg bg-muted" />
        )}
      </DialogContent>
    </Dialog>
  );
