  ...
  const visitsById = useMemo(() => {
    ...
    for (const c of customers) {
      ...
      const count =
        ...
        bookings.filter(
          (b) => matchesCustomer(who, b.customer_name, b.phone) && isFinancialBooking(b),
        ).length +
        sales.filter((s) => matchesCustomer(who, s.customer_name, null) && isFinancialSale(s))
          .length;
      map.set(c.id, count);
    }
    return map;
            ...
            {filtered.length > CUSTOMER_PAGE_SIZE && (
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                <span>
                  ...
                  {Math.min(page * CUSTOMER_PAGE_SIZE, filtered.length)} of {filtered.length}
                </span>
