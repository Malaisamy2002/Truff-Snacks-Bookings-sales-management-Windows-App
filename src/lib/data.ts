...
export function useMergeCustomers() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      // Re-tagging uses the same phone-first / name-fallback identity rule as
      // everywhere else (`matchesCustomer` — see its doc comment). This used
      // to reimplement its own looser "name match OR phone match" check
      // instead, which is a real data-integrity bug: when both sides have a
      // phone, matchesCustomer compares ONLY the phone (two different people
      // who share a name must never collapse into one row) — the old OR
      // logic here would still match on name alone and silently reassign an
      // unrelated customer's bills to the merged identity. And when a bill's
      // phone coincidentally matched the absorbed customer's phone (e.g. two
      // family members booking under different names but the same contact
      // number — plausible for this app), the old logic reassigned that
      // OTHER person's records too, regardless of name. Routing through
      // matchesCustomer closes both directions and keeps this function
      // honest with its own documentation, which already claimed it used
      // the shared rule.
      for (const c of payload.absorb) {
        ...
        for (const s of sales) {
          // Snack sales don't reliably carry a phone (see SnackSale's
          // optional `phone`), so fall back to name-only matching here —
          // same as before, just no longer duplicated ad hoc.
          if (normName(s.customer_name as string) === normName(c.name))
