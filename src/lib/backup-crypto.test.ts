...
describe("backup-crypto", () => {
  it("round-trips arbitrary bytes under the right passphrase", async () => {
    const plaintext = bytesOf("every customer, bill, expense and receipt photo");
    const container = await encryptBackup(plaintext, "correct horse battery staple");
    const restored = await decryptBackup(container, "correct horse battery staple");
  ...
  it("rejects the wrong passphrase instead of returning garbage", async () => {
    const container = await encryptBackup(bytesOf("secret ledger data"), "right-pw");
    await expect(decryptBackup(container, "wrong-pw")).rejects.toBeInstanceOf(WrongPassphraseError);
  });
  ...
  it("rejects a tampered container (GCM auth check)", async () => {
    ...
    await expect(decryptBackup(tampered, "pw")).rejects.toBeInstanceOf(WrongPassphraseError);
  });
  ...
  it("isEncryptedBackup identifies containers and rejects plain data", async () => {
    ...
    expect(isEncryptedBackup(bytesOf("PK\x03\x04 a plain zip file"))).toBe(false);
    expect(isEncryptedBackup(new Uint8Array(0))).toBe(false);
  });

  it("rejects decrypting bytes that aren't a container at all", async () => {
    await expect(decryptBackup(bytesOf("not a container"), "pw")).rejects.toThrow(
