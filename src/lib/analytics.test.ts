...
describe("dataDateRange()", () => {
  ...
  it("reads bill_date as a full IST timestamp, not a UTC slice", () => {
    ...
    const range = dataDateRange(src({ bills: [bill({ bill_date: "2026-01-01T19:00:00.000Z" })] }));
...
describe("itemPerformance()", () => {
  ...
  it("ignores sales outside the period and merged-into-bill sales", () => {
    const inPeriodItems = [
      { item_name: "Chips", qty: 3, unit_price: 20, cost_price: 10, amount: 60 },
    ];
    ...
    const perf = itemPerformance([inPeriod, outOfPeriod, merged], (iso) => iso === "2026-09-01");
