...
import { compressReceiptImage, isLikelyImageFile, sniffImageMimeType } from "./image";
...
describe("compressReceiptImage()", () => {
  ...
  it("passes a file with no MIME type through unchanged", async () => {
    const file = new File([new Uint8Array([1, 2, 3])], "mystery.bin", { type: "" });
    const result = await compressReceiptImage(file);
    expect(result).toBe(file);
  });
});
...
describe("sniffImageMimeType()", () => {
  it("recognizes a JPEG signature", () => {
    expect(sniffImageMimeType(new Uint8Array([0xff, 0xd8, 0xff, 0xe0, 0, 0]))).toBe("image/jpeg");
  });
...
describe("isLikelyImageFile()", () => {
  it("accepts a file with real JPEG bytes even if named oddly", async () => {
    ...
    const file = new File([bytes], "whatever.dat", { type: "application/octet-stream" });
