...
const emptyTables = () => Object.fromEntries(DATA_TABLES.map((t) => [t, []]));
...
describe("findInvalidRows()", () => {
  ...
  it("accepts a structurally sound row", () => {
    ...
    tables.customers = [{ id: "c1", name: "Ravi", phone: null, created_at: "2026-09-04" }];
    tables.expenses = [
      {
        id: "e1",
        business: "Turf",
  ...
  it("collects problems across multiple tables, not just the first one hit", () => {
    ...
    const problems = findInvalidRows(tables);
    expect(problems.map((p) => p.table).sort()).toEqual(["customers", "snack_combos"]);
  });
  ...
  it("reports one problem per bad row, not one per bad field", () => {
    ...
    tables.expenses = [{ business: "Turf", category: "Maintenance", amount: "bad", spent_at: "x" }];
...
describe("describeInvalidRows()", () => {
  ...
  it("uses singular phrasing for exactly one problem", () => {
    const message = describeInvalidRows([{ table: "customers", index: 0, reason: "x" }]);
...
describe("findInvalidReceiptHashRows()", () => {
  it("accepts a well-shaped hash row", () => {
    expect(
      findInvalidReceiptHashRows([{ path: "Receipts/2026-09-04/x.jpg", sha256: "abc123" }]),
    ).toEqual([]);
  });

  it("flags a hash row missing sha256", () => {
    const problems = findInvalidReceiptHashRows([{ path: "Receipts/2026-09-04/x.jpg" }]);
    expect(problems).toHaveLength(1);
...
describe("findInvalidPhotoRows()", () => {
  it("accepts a well-shaped photo row", () => {
    expect(
      findInvalidPhotoRows([
        { path: "Receipts/2026-09-04/x.jpg", data: "base64==", created_at: "2026-09-04" },
      ]),
    ).toEqual([]);
  });
  ...
  it("flags a photo row with a non-string data field", () => {
    const problems = findInvalidPhotoRows([
      { path: "Receipts/2026-09-04/x.jpg", data: 12345, created_at: "2026-09-04" },
    ]);
    expect(problems).toHaveLength(1);
