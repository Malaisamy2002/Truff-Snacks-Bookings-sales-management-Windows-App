...
type FieldCheck = readonly [field: string, check: (v: unknown) => boolean, mode?: "optional"];
 ...
 * messages.
 */
function checkRows(rows: unknown[], checks: readonly FieldCheck[]): RowProblem[] {
  ...
  rows.forEach((row, index) => {
    ...
    for (const [field, check, mode] of checks) {
      ...
      if (!check(value)) {
        problems.push({ index, reason: `"${field}" is missing or the wrong type` });
        return;
      }
    }
  });
 ...
 */
export function findInvalidRows(tables: Record<string, unknown[] | undefined>): InvalidRow[] {
  const problems: InvalidRow[] = [];
  for (const t of DATA_TABLES) {
    const rows = tables[t] ?? [];
    for (const p of checkRows(rows, TABLE_CHECKS[t])) problems.push({ table: t, ...p });
  }
  return problems;
}
...
export function describeInvalidRows(problems: InvalidRow[]): string {
  ...
  for (const p of problems) byTable.set(p.table, (byTable.get(p.table) ?? 0) + 1);
  const parts = [...byTable.entries()].map(([t, n]) => `${n} in ${t}`).join(", ");
