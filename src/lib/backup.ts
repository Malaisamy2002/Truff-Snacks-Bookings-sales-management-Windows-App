import { db, table, DATA_TABLES, type DataTable, type Row, type ReceiptHashRow } from "./localdb";
import {
  isAndroid,
  isDesktop,
  saveExportFile,
...
import { encryptFullBackupBytes, decryptFullBackupBytes } from "./backup-crypto";
...
export async function restoreBackup(backup: BackupFile, mode: "replace" | "merge" = "replace") {
  ...
  const mismatchedPhotos = await findHashMismatchedPhotos(backup.photos ?? [], receiptHashes);
    ...
    async () => {
      ...
      if (receiptHashes.length > 0) {
        if (mode === "merge") {
          ...
          const freshHashes = receiptHashes.filter((h) => !existingHashPaths.has(h.path));
