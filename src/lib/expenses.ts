...
export async function uploadReceipt(file: File, date: string = dayKey(new Date())) {
  ...
  if (!(await isLikelyImageFile(file))) {
    throw new Error("That file doesn't look like an image and can't be saved as a receipt.");
  }
...
export async function openReceipt(path: string): Promise<string | null> {
  if (isAndroid()) {
    ...
    if (!result.saved) throw new Error(result.error ?? "Could not open this receipt");
